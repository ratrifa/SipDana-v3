        ...
        <Modal.Body className="px-4 pb-4 pt-3">
          ...
          <Form onSubmit={handleSubmit}>
            <Form.Group className="mb-4">
              <Form.Label className="fw-semibold small text-muted mb-2">Nominal Nabung (Rp)</Form.Label>
              <Form.Control
                type="text"
                ...
                required
              />
              <p className="text-center small text-muted mt-2 mb-0">Saldo utama Anda akan dialokasikan ke target ini secara virtual.</p>
            </Form.Group>

            <div className="d-flex gap-2">
              <Button variant="primary" type="submit" disabled={loading} className="flex-fill py-3 fw-bold shadow-sm" style={{ borderRadius: "15px" }}>
        ...
        <Modal.Body className="px-4 pb-4 pt-3">
          ...
          <Form onSubmit={handleSubmit}>
            <Form.Group className="mb-3">
              ...
              <Form.Control type="text" name="nama_target" value={formData.nama_target} onChange={handleChange} style={inputStyle} placeholder="Misal: Liburan Akhir Tahun" required />
            </Form.Group>
            ...
            <div className="row g-3 mb-3">
              <div className="col-6">
                <Form.Group>
                  ...
                  <Form.Control type="text" name="target_jumlah" value={formData.target_jumlah} onChange={handleChange} style={inputStyle} placeholder="0" required />
                </Form.Group>
              </div>
              <div className="col-6">
                <Form.Group>
                  <Form.Label className="fw-semibold small text-muted mb-2">Target Tanggal</Form.Label>
                  <Form.Control type="date" name="tanggal_target" value={formData.tanggal_target} onChange={handleChange} style={inputStyle} required />
                </Form.Group>
              </div>
            </div>
...
const TargetItem: React.FC<{ target: TargetTypes.TargetMenabung; onContribute: (t: TargetTypes.TargetMenabung) => void }> = ({ target, onContribute }) => {
  ...
  return (
    <Card className={`shadow-sm border-0 h-100 mb-4`} style={{ borderRadius: "25px", overflow: "hidden" }}>
      <Card.Body className="p-4">
        ...
        <p className="text-muted small mb-4">Sampai: {new Date(target.tanggal_target).toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric" })}</p>
        ...
        <ProgressBar now={progress} variant={isAchieved ? "success" : "primary"} className="mb-2 shadow-sm" style={{ height: "12px", borderRadius: "10px" }} animated={!isAchieved && progress > 0} />

        <div className="d-flex justify-content-between align-items-center">
          <span className={`fw-bold ${isAchieved ? "text-success" : "text-primary"}`}>{progress.toFixed(0)}%</span>
          <span className="text-muted small">{isAchieved ? "Hebat! Target tercapai" : `Sisa ${formatRupiah(target.target_jumlah - target.jumlah_terkumpul)} lagi`}</span>
        </div>
      </Card.Body>

      <Card.Footer className="bg-transparent border-0 p-4 pt-0">
        <Button variant={isAchieved ? "outline-success" : "primary"} className="w-100 py-2 fw-bold" disabled={isAchieved} onClick={() => onContribute(target)} style={{ borderRadius: "15px" }}>
...
const TargetMenabungPage = () => {
  ...
  const loadTargets = useCallback(async () => {
    setLoading(true);
    try {
      const [targetData, summaryData] = await Promise.all([fetchActiveTargets(), fetchMonthlySummary()]);
  ...
  return (
    <MainLayout onTransactionAdded={loadTargets} openTransactionModal={() => setShowTransactionModal(true)}>
      <h2 className="mb-4 d-flex align-items-center text-primary fw-bold" style={{ fontSize: "35px" }}>
        <img src={IconTargetBiru} alt="Ikon Target" className="me-2" style={{ width: "32px", height: "32px", display: "block", marginTop: "-1px" }} />
        <span style={{ display: "inline-block", lineHeight: "1.2" }}>Target Menabung</span>
      </h2>
      ...
      {loading ? (
        ...
        <>
          <Row className="mb-4">
            <Col md={6} className="mb-3">
              <Card className="shadow-sm border-0 h-100 p-4" style={{ borderRadius: "20px" }}>
                ...
                <div className="d-flex justify-content-between align-items-center">
                  <h3 className="fw-bold text-primary mb-0">{showSaldo ? formatRupiah(totalSaldo) : "Rp ••••••"}</h3>
                  <div onClick={() => setShowSaldo(!showSaldo)} style={{ cursor: "pointer" }} className="text-muted p-2">
            ...
            <Col md={6} className="mb-3">
              ...
              >
                ...
                <h3 className="fw-bold mb-1">{showSaldo ? formatRupiah(totalSaldo * 0.2) : "Rp ••••••"}</h3>
          ...
          <div className="d-grid mb-5">
            <Button variant="primary" size="lg" className="py-3 fw-bold shadow" onClick={() => setShowModal(true)} style={{ borderRadius: "30px", border: "none", fontSize: "20px" }}>
              Buat Target Baru
            </Button>
          </div>
          ...
          {targets.length === 0 ? (
            <Card className="shadow-sm border-0 p-5 text-center mt-2" style={{ borderRadius: "25px", overflow: "hidden" }}>
              <img src={OnlyLogoBiru} alt="Logo SipDana" className="img-fluid mb-4 mx-auto" style={{ maxWidth: "300px" }} />
              <h4 className="fw-bold text-secondary">Belum Ada Target Aktif</h4>
              <p className="text-muted mb-4">
                Miliki rencana masa depan yang lebih tertata.
                <br />
