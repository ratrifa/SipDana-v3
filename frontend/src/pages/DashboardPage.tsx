...
const DashboardPage = () => {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [summaryData, historyData, historicalData] = await Promise.all([fetchMonthlySummary(), fetchTransactionHistory(), fetchHistoricalData()]);
  ...
  return (
    <MainLayout onTransactionAdded={handleModalSuccess} openTransactionModal={() => setShowModal(true)} hideAddButton={true}>
      <h2 className="mb-4 d-flex align-items-center text-primary fw-bold" style={{ fontSize: "35px" }}>
        ...
        />
        <span style={{ display: "inline-block", lineHeight: "1.2" }}>Beranda</span>
      </h2>
      ...
      {hasData ? (
          ...
          <Row className="mb-4">
            <Col md={6} className="mb-3">
              <Card className="shadow-sm border-0 h-100 p-4" style={{ borderRadius: "20px" }}>
                ...
                <p className={`small mt-2 d-flex align-items-center ${totalSelisih > 0 ? "text-success" : totalSelisih < 0 ? "text-danger" : "text-muted"}`}>
            ...
            <Col md={6} className="mb-3">
              <Card className="shadow-sm border-0 h-100 border-start border-warning border-4" style={{ backgroundColor: "#fffbe6", borderRadius: "20px" }}>
                <Card.Body>
                  ...
                  <p className="text-dark small mb-2">{statusInfo.advice}</p>
                </Card.Body>
              </Card>
            </Col>
          </Row>
          ...
          <Card className="shadow-sm border-0 mb-5" style={{ borderRadius: "25px", overflow: "hidden" }}>
            <Card.Body className="p-4">
              <div className="d-flex justify-content-between align-items-center mb-4">
                <h4 className="fw-bold text-muted mb-0" style={{ color: "#000" }}>
        ...
        <Card className="shadow-sm border-0 p-5 text-center mt-5" style={{ borderRadius: "20px", overflow: "hidden" }}>
          <img src={OnlyLogoBiru} alt="Logo SipDana" className="img-fluid mb-4 mx-auto" style={{ maxWidth: "350px" }} />
          <h4 className="text-secondary">Selamat Datang di SipDana!</h4>
          <p className="text-muted mb-4">Ayo mulai kelola keuangan Anda dengan mencatat transaksi pertama.</p>
          <Button
            variant="primary"
            size="lg"
            onClick={() => setShowModal(true)}
