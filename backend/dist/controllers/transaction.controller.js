"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.createTransaction = void 0;
const db_config_1 = __importDefault(require("../config/db.config"));
/**
 * [POST] Mencatat Transaksi Baru (Pemasukan/Pengeluaran)
 * Endpoint: POST /api/transactions
 */
const createTransaction = async (req, res) => {
    const userId = req.user?.id_user;
    if (!userId)
        return res.status(401).json({ message: 'User ID tidak ditemukan.' });
    const { jenis, jumlah, tanggal, keterangan, id_kategori } = req.body;
    if (!jenis || !jumlah || !tanggal || !keterangan || !id_kategori) {
        return res.status(400).json({ message: 'Semua field transaksi wajib diisi.' });
    }
    if (jumlah <= 0) {
        return res.status(400).json({ message: 'Jumlah transaksi harus positif.' });
    }
    try {
        await db_config_1.default.query('START TRANSACTION');
        const transactionQuery = `
            INSERT INTO transaksi 
            (id_user, jenis, jumlah, tanggal, keterangan, id_kategori) 
            VALUES (?, ?, ?, ?, ?, ?)
        `;
        const [result] = await db_config_1.default.query(transactionQuery, [
            userId, jenis, jumlah, tanggal, keterangan, id_kategori
        ]);
        const transactionId = result.insertId;
        const amount = jenis === 'pemasukan' ? jumlah : -jumlah;
        const [saldoRows] = await db_config_1.default.query('SELECT id_saldo FROM saldo WHERE id_user = ?', [userId]);
        if (saldoRows.length > 0) {
            await db_config_1.default.query('UPDATE saldo SET saldo_sekarang = saldo_sekarang + ? WHERE id_user = ?', [amount, userId]);
        }
        else {
            await db_config_1.default.query('INSERT INTO saldo (id_user, saldo_sekarang) VALUES (?, ?)', [userId, amount]);
        }
        await db_config_1.default.query('COMMIT');
        res.status(201).json({
            message: 'Transaksi berhasil dicatat dan saldo diperbarui.',
            transactionId
        });
    }
    catch (error) {
        await db_config_1.default.query('ROLLBACK');
        console.error('Error saat membuat transaksi:', error);
        res.status(500).json({ message: 'Gagal mencatat transaksi dan memperbarui saldo.' });
    }
};
exports.createTransaction = createTransaction;
